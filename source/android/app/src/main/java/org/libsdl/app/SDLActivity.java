...
import net.cubers.assaultcube.AssetExporter;
import net.cubers.assaultcube.AssaultCubeLib;
...
public class SDLActivity extends Activity implements View.OnSystemUiVisibilityChangeListener {
    ...
    // OUYA/Tegra3 perf: max height (px) of the render surface buffer; the display upscales it to the
    // actual screen. 0 = render at native screen resolution. 720 = 1280x720 on a 1080p OUYA (~2.25x
    // fewer pixels). Drop to 540 (960x540) if still choppy, raise toward 1080 for more sharpness.
    public static int sRenderMaxHeight = 720;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // OUYA/Tegra3 perf: when we shrink the render buffer via getHolder().setFixedSize() (sRenderMaxHeight),
        // the default WRAP_CONTENT params make the SurfaceView measure itself to the *buffer* size, so the
        // Nvidia HWC composited it as a 1280x720 overlay in the top-left corner. Force MATCH_PARENT so the
        // view always fills the screen and the compositor upscales the smaller buffer to fullscreen.
    ...
    public void superOnPause() { super.onPause(); }
    ...
    public void superOnStop() { super.onStop(); }
...
class SDLSurface extends SurfaceView implements SurfaceHolder.Callback,
    ...
    public void surfaceChanged(SurfaceHolder holder,
        ...
        // OUYA/Tegra3 perf: the ULP GeForce is fillrate/bandwidth-bound and the OUYA surface is full 1080p.
        // Render into a smaller buffer and let the display hardware upscale it to the screen. setFixedSize()
        // re-triggers surfaceChanged() at the reduced size, so we apply it once (while the incoming size is
        // still bigger than the target) and bail out; the engine then sees the reduced size as scr_w/scr_h.
        if (SDLActivity.sRenderMaxHeight > 0 && height > SDLActivity.sRenderMaxHeight) {
            int targetH = SDLActivity.sRenderMaxHeight;
            int targetW = Math.round((float) width * targetH / height) & ~1; // keep even
            Log.v("SDL", "perf: downscaling render surface " + width + "x" + height + " -> " + targetW + "x" + targetH);
