...
#ifdef __ANDROID__
#include <stdlib.h>
// gl4es reads its LIBGL_* configuration in a constructor of priority 101 that runs at
// libmain.so load time (before main()). We must therefore set these env vars from an
// EARLIER constructor (priority 100). On the OUYA's Tegra 3, the Cg shader compiler
// (libcgdrv.so CgDrv_Compile) SIGSEGVs on gl4es' default fixed-pipeline shaders; forcing
// mediump (no highp) and disabling the precompiled-shader-assembly cache avoids it.
__attribute__((constructor(100)))
static void ouya_gl4es_env(void)
{
    // LIBGL_NOTEST: skip gl4es' hardware-probing test shaders. Those probes compile GLSL with
    // ES3/desktop syntax (layout(location=...) / "in" qualifiers) which makes the Tegra 3 Cg
    // shader compiler (libcgdrv.so) SIGSEGV. notest also skips gl4es' own EGL pbuffer probe
    // context (the EGL_BAD_DISPLAY source) and just assumes sane GLES2 defaults.
    setenv("LIBGL_NOTEST", "1", 1);
    setenv("LIBGL_NOHIGHP", "1", 1);        // force mediump in fragment shaders (Tegra 3)
    setenv("LIBGL_NOPSA", "1", 1);          // disable precompiled shader assembly cache
    setenv("LIBGL_NOBANNER", "1", 1);
    setenv("LIBGL_LOGSHADERERROR", "1", 1); // log any shader that fails to compile
    setenv("LIBGL_ES", "2", 1);             // target GLES2 explicitly
    // perf (Tegra 3 is fillrate/bandwidth-bound):
    setenv("LIBGL_MIPMAP", "3", 1);         // auto-generate + use mipmaps: fewer texture-cache misses on minification
    setenv("LIBGL_NOERROR", "1", 1);        // skip gl4es' internal glGetError checks (driver/CPU overhead)
}
#endif
...
#ifdef __ANDROID__
    {
        defvformatstring(amsg,s,s);
        SDL_Log("AC FATAL: %s (SDL_GetError: %s)", amsg, SDL_GetError());
    }
#endif
...
void mapscreenshot(const char *imagepath, bool mapshot, int fileformat, float scale, int height, int quality)
{
    ...
    int src_w = mapshot ? minimaplastsize : scrw;
    int src_h = mapshot ? minimaplastsize : scrh;
...
void screenres(int w, int h)
{
    ...
    SDL_Window *wdow = SDL_CreateWindow("AssaultCube",
                                          SDL_WINDOWPOS_UNDEFINED,
                                          SDL_WINDOWPOS_UNDEFINED,
                                          w, h,
                                          SDL_WINDOW_OPENGL|SDL_WINDOW_RESIZABLE|(SDL_GetWindowFlags(screen)&SDL_WINDOW_FULLSCREEN));
    glcontext = SDL_GL_CreateContext(screen); // error handling
    if(!wdow) return;
    screen = wdow;
    int scrw, scrh;
    SDL_GetWindowSize(screen, &scrw, &scrh);
...
VARNFP(gamma, vgamma, 30, 100, 300,
{
    ...
    if(SDL_SetWindowBrightness(screen, f)==-1) conoutf("Could not set gamma: %s", SDL_GetError());
});

void cleargamma()
{
    if(curgamma != 100) SDL_SetWindowBrightness(screen, 1);
}
...
void restoregamma()
{
    ...
    SDL_SetWindowBrightness(screen, 1);
    SDL_SetWindowBrightness(screen, f);
}
...
void getdisplayresolutions()
{
    string res = "";
    int lh = 0, lw = 0;
    // fixme ah, proper display?
    int primaryScreen = 0;
    int numModes = SDL_GetNumDisplayModes(primaryScreen);
    for(int modeIdx = 0; modeIdx < numModes; modeIdx++)
    {
        SDL_DisplayMode mode = { SDL_PIXELFORMAT_UNKNOWN, 0, 0, 0, 0 };
        if(SDL_GetDisplayMode(primaryScreen, modeIdx, &mode)==0)
        {
            if(lw != mode.w && lh != mode.h) concatformatstring(res, "%s%d %d", *res ? " " : "", (lw = mode.w), (lh = mode.h));
        }
    }
    result(res);
}
...
void setupscreen(int &usedcolorbits, int &useddepthbits, int &usedfsaa)
{
    ...
    // fixme: migration to SDL2 has dropped colorbits support
    //if(colorbits)
    //    hasbpp = SDL_VideoModeOK(mode.format != SDL_PIXELFORMAT_UNKNOWN ? mode.w : scr_w, mode.format != SDL_PIXELFORMAT_UNKNOWN ? mode.h : scr_h, colorbits, SDL_WINDOW_OPENGL|flags)==colorbits;

    SDL_GL_SetAttribute(SDL_GL_DOUBLEBUFFER, 1);
#ifdef __ANDROID__
    // OUYA / Tegra 3 is OpenGL ES 2.0 only: explicitly request an ES 2.0 context so SDL can
    // select a compatible EGLConfig (otherwise SDL asks for desktop GL -> EGL_BAD_DISPLAY).
    // gl4es translates the engine's immediate-mode GL 1.x/2.x calls onto this GLES2 context.
    ...
    loopi(sizeof(configs)/sizeof(configs[0]))
    {
        ...
        // fixme: migration to SDL2 has dropped colorbits support
        //window = SDL_SetVideoMode(scr_w, scr_h, hasbpp ? colorbits : 0, SDL_WINDOW_OPENGL|flags);
        ...
        if(screen)
        {
            glcontext = SDL_GL_CreateContext(screen); // error handling
#ifdef __ANDROID__
            SDL_Log("AC setupscreen: config=%d screen=%p glcontext=%p err=%s", config, (void*)screen, (void*)glcontext, SDL_GetError());
#endif
            break;
        }
#ifdef __ANDROID__
        else SDL_Log("AC setupscreen: SDL_CreateWindow failed config=%d err=%s", config, SDL_GetError());
#endif
    }
    if(!screen) fatal("Unable to create OpenGL screen");
#ifdef __ANDROID__
    if(!glcontext) fatal("Unable to create OpenGL ES context");
#endif
    else
    {
    ...
    SDL_SetRelativeMouseMode(fullscreen ? SDL_TRUE : SDL_FALSE);
...
static void resetmousemotion()
{
    ...
    if(!(SDL_GetWindowFlags(screen)&SDL_WINDOW_FULLSCREEN ))
    {
        int scrw, scrh;
        SDL_GetWindowSize(screen, &scrw, &scrh);
        SDL_WarpMouseInWindow(screen, scrw / 2, scrh / 2);
    }
#endif
}
...
static inline bool skipmousemotion(SDL_Event &event)
{
    ...
    if(!(SDL_GetWindowFlags(screen)&SDL_WINDOW_FULLSCREEN))
    {
        ...
        int scrw, scrh;
        SDL_GetWindowSize(screen, &scrw, &scrh);
        if(event.motion.x == scrw / 2 && event.motion.y == scrh / 2) return true;  // ignore any motion events generated SDL_WarpMouse
    }
#endif
    return false;
}
...
// ---------------- Game controller (OUYA / Android gamepad) ----------------
// AssaultCube is a keyboard+mouse FPS with no native gamepad support, and the OUYA has no
// touchscreen, so the bundled Android touch UI is useless here. We translate an SDL
// GameController to the engine's existing action commands (forward/backward/left/right/jump/
// crouch/attack/...), mouse-look via mousemove(), and — while an input menu is up — to the
// navigation keysyms the menu already understands. Polled every frame from checkinput().
VARP(gamepadlooksens, 1, 40, 300);      // right-stick look speed (tunable, persisted)
VARP(gamepaddeadzone, 0, 8000, 30000);  // analog stick deadzone in raw axis units
...
static void opengamecontroller()
{
    ...
    for(int i = 0; i < n; i++) if(SDL_IsGameController(i))
    {
        gamecontroller = SDL_GameControllerOpen(i);
        if(gamecontroller) { SDL_Log("AC gamepad opened: %s", SDL_GameControllerName(gamecontroller)); return; }
    }
}

static inline int padadj(int v)  // remove deadzone
{
    if(v >  gamepaddeadzone) return v - gamepaddeadzone;
    if(v < -gamepaddeadzone) return v + gamepaddeadzone;
    return 0;
}

// The engine's "d" (down-state) commands take their bool from the key-press context, not from
// execute() arguments (see command.cpp: it passes addreleaseaction()!=NULL, which is NULL outside a
// key event), so execute("forward 1") is a no-op. We must call these functions directly instead.
extern void forward(bool); extern void backward(bool); extern void left(bool); extern void right(bool);
extern void attack(bool);  extern void jumpn(bool);    extern void crouch(bool);

static void padmovef(void (*fn)(bool), bool &prev, bool down)  // direct "d"-action, re-asserted while held
{
    if(down) fn(true);
    else if(prev) fn(false);
    prev = down;
}

// Held action. On Android checktouchinput() runs every frame *before* us and resets movement to
// neutral, so an edge-only "cmd 1" would be cancelled on the next frame. We therefore RE-ASSERT
// "cmd 1" every frame while the input is held, and issue "cmd 0" once on release.
static void padmove(const char *cmd, bool &prev, bool down)
{
    if(down) { defformatstring(c)("%s 1", cmd); execute(c); }
    else if(prev) { defformatstring(c)("%s 0", cmd); execute(c); }
    prev = down;
}

static void padtap(const char *cmd, bool &prev, bool now)  // run a console command once per press edge
{
    if(now && !prev) execute(cmd);
    prev = now;
}

static void menutap(bool &prev, bool now, int sym)  // one keysym tap per press edge, for menus
{
    if(now && !prev) { keypress(sym, true, sym); keypress(sym, false, sym); }
    prev = now;
}

// OUYA / Android gamepad mapping (polled every frame from the main loop).  Coherent console-FPS layout:
//   Left stick   = move (forward/back/strafe)      Right stick = look
//   R2 = fire     L2 = aim/scope                    R1 = next weapon   L1 = previous weapon
//   A = jump      B = crouch                        X = reload         Y = knife (melee)
//   D-pad: Up = grenade, Down = pistol, Left = prev weapon, Right = next weapon
//   Back/Select = scoreboard (hold)                Start = open/close menu
// In a menu: D-pad or left stick navigate, A = confirm, B/Start/Back = back.
void processgamepad()
{
    static int retry = 0;
    if(!gamecontroller)
    {
        if(--retry > 0) return;
        retry = 90;                 // probe for a pad roughly every 90 frames until found
        opengamecontroller();
        if(!gamecontroller) return;
    }
    if(!SDL_GameControllerGetAttached(gamecontroller))
    {
        SDL_GameControllerClose(gamecontroller); gamecontroller = NULL; return;
    }
    SDL_GameController *gc = gamecontroller;
    extern bool gamepadmenuopen();
    bool inmenu = gamepadmenuopen();

    #define PADBTN(b) (SDL_GameControllerGetButton(gc, b) != 0)
    bool a = PADBTN(SDL_CONTROLLER_BUTTON_A), b = PADBTN(SDL_CONTROLLER_BUTTON_B);
    bool x = PADBTN(SDL_CONTROLLER_BUTTON_X), y = PADBTN(SDL_CONTROLLER_BUTTON_Y);
    bool l1 = PADBTN(SDL_CONTROLLER_BUTTON_LEFTSHOULDER), r1 = PADBTN(SDL_CONTROLLER_BUTTON_RIGHTSHOULDER);
    bool start = PADBTN(SDL_CONTROLLER_BUTTON_START), back = PADBTN(SDL_CONTROLLER_BUTTON_BACK);
    bool guide = PADBTN(SDL_CONTROLLER_BUTTON_GUIDE);   // OUYA centre button (usually swallowed by the OS, so unreliable)
    bool r3 = PADBTN(SDL_CONTROLLER_BUTTON_RIGHTSTICK); // stick-clicks DO work on OUYA (centre button does not)
    bool l3 = PADBTN(SDL_CONTROLLER_BUTTON_LEFTSTICK);  // OUYA: hold L3 to show the scoreboard (no physical Back button)
    bool up = PADBTN(SDL_CONTROLLER_BUTTON_DPAD_UP), dn = PADBTN(SDL_CONTROLLER_BUTTON_DPAD_DOWN);
    bool lf = PADBTN(SDL_CONTROLLER_BUTTON_DPAD_LEFT), rt = PADBTN(SDL_CONTROLLER_BUTTON_DPAD_RIGHT);


    int lx = padadj(SDL_GameControllerGetAxis(gc, SDL_CONTROLLER_AXIS_LEFTX));
    int ly = padadj(SDL_GameControllerGetAxis(gc, SDL_CONTROLLER_AXIS_LEFTY));
    int rx = padadj(SDL_GameControllerGetAxis(gc, SDL_CONTROLLER_AXIS_RIGHTX));
    int ry = padadj(SDL_GameControllerGetAxis(gc, SDL_CONTROLLER_AXIS_RIGHTY));
    bool rtrig = SDL_GameControllerGetAxis(gc, SDL_CONTROLLER_AXIS_TRIGGERRIGHT) > 8000;
    bool ltrig = SDL_GameControllerGetAxis(gc, SDL_CONTROLLER_AXIS_TRIGGERLEFT) > 8000;

    // held-action states
    static bool mFwd=false, mBack=false, mLeft=false, mRight=false, mFire=false, mJump=false, mCrouch=false, mScope=false, mScores=false;
    // press-edge states
    static bool eUp=false, eDn=false, eLf=false, eRt=false, eA=false, eB=false, eX=false, eY=false, eL1=false, eR1=false, eStart=false;
    static bool padmenugate = false;   // OUYA: swallow held buttons for one release after leaving a menu (anti input-bleed)

    // OUYA: R3 (right-stick click) TOGGLES the menu. The round centre button is swallowed by the OUYA OS
    // (never reaches SDL), so R3 is the reliable opener. In a menu R3 = ESCAPE (go back one level / close);
    // in-game R3 = open the in-game menu. Single edge var so holding it can't open-then-immediately-close.
    static bool eMenuToggle = false;
    bool menutoggle = r3 || guide;   // guide kept in case some OUYA firmware does expose the centre button
    if(menutoggle && !eMenuToggle)
    {
        if(inmenu) { keypress(SDLK_ESCAPE, true, SDLK_ESCAPE); keypress(SDLK_ESCAPE, false, SDLK_ESCAPE); } // back/close
        else execute("ingamemenu");                                                                        // open
    }
    eMenuToggle = menutoggle;

    if(inmenu)
    {
        // release any held gameplay actions while a menu captures input
        padmovef(forward, mFwd, false);  padmovef(backward, mBack, false);
        padmovef(left, mLeft, false);    padmovef(right, mRight, false);
        padmovef(attack, mFire, false);  padmovef(jumpn, mJump, false);
        padmovef(crouch, mCrouch, false);
        padmove("setscope", mScope, false); padmove("showscores", mScores, false);
        padmenugate = true;   // we were in a menu this frame; swallow still-held buttons after it closes

        // OUYA text entry: when the selected item is a text field (nickname, custom IP, ...) A pops the
        // Android soft keyboard (SDL_StartTextInput). While it is up we suspend menu navigation so the
        // OUYA IME owns the controller; A or B closes it again. Typed chars are routed in checktouchinput.
        extern bool menutextinputfocused();
        extern void menutextinputcommit();
        // Commit the field whenever the soft keyboard closes (ENTER on the IME, or our close button below),
        // otherwise the typed nickname / custom IP is never applied.
        static bool wasediting = false;
        bool editingnow = SDL_IsTextInputActive();
        if(wasediting && !editingnow) menutextinputcommit();
        wasediting = editingnow;

        static bool eEditBtn = false;
        if(SDL_IsTextInputActive())
        {
            if(!menutextinputfocused()) { SDL_StopTextInput(); }   // selection moved off the field
            else
            {
                bool closebtn = a || b;
                if(closebtn && !eEditBtn) SDL_StopTextInput();
                eEditBtn = closebtn;
                return;   // keyboard is up: let the IME have the pad
            }
        }
        if(menutextinputfocused() && a && !eA)
        {
            SDL_StartTextInput();     // open the soft keyboard on this field
            eA = true; eEditBtn = true;
            return;
        }

        // D-pad or left stick navigate; A confirms, B / Start / Back / centre button go back/close
        menutap(eUp, up || ly < -16000, SDLK_UP);
        menutap(eDn, dn || ly >  16000, SDLK_DOWN);
        menutap(eLf, lf || lx < -16000, SDLK_LEFT);
        menutap(eRt, rt || lx >  16000, SDLK_RIGHT);
        // A = confirm: the classic menu activates an item on a LEFT-MOUSE "click" (SDL_AC_BUTTON_LEFT),
        // NOT on SDLK_RETURN (AC's menukey only accepts SDL_SCANCODE_RETURN / mouse-left), so send a click.
        menutap(eA, a, SDL_AC_BUTTON_LEFT);
        menutap(eB, b, SDLK_ESCAPE);
        menutap(eStart, start || back, SDLK_ESCAPE);
        return;
    }

    // OUYA: input-bleed guard. When a menu item is confirmed with A the menu often closes the SAME frame;
    // if we resumed gameplay immediately the still-held A would register as a JUMP. So after leaving a menu
    // we keep gameplay neutral until the player releases the menu buttons (A / B / R3 / start / back).
    if(padmenugate)
    {
        if(a || b || r3 || guide || start || back)
        {
            padmovef(forward, mFwd, false);  padmovef(backward, mBack, false);
            padmovef(left, mLeft, false);    padmovef(right, mRight, false);
            padmovef(attack, mFire, false);  padmovef(jumpn, mJump, false);
        ...
        // all menu buttons released: reset shared press-edges (they were used for menu nav) and resume
        eUp=eDn=eLf=eRt=eA=eB=eX=eY=eL1=eR1=eStart=false;
        padmenugate = false;
    }

    // ---- in-game (re-assert held actions every frame so the touch layer can't cancel them) ----
    padmovef(forward,  mFwd,   ly < 0);
    padmovef(backward, mBack,  ly > 0);
    padmovef(left,     mLeft,  lx < 0);
    padmovef(right,    mRight, lx > 0);
    padmovef(attack,   mFire,  rtrig);
    padmovef(jumpn,    mJump,  a);
    padmovef(crouch,   mCrouch, b);
    padmove("setscope", mScope, ltrig);
    padmove("showscores", mScores, back || l3);   // OUYA: hold L3 (or Back) to show the scoreboard
    // press-edge actions
    padtap("reload",          eX, x);
    padtap("weapon knife",    eY, y);
    padtap("shiftweapon -1",  eL1, l1);
    padtap("shiftweapon 1",   eR1, r1);
    padtap("weapon grenade",  eUp, up);
    padtap("weapon pistol",   eDn, dn);
    padtap("shiftweapon -1",  eLf, lf);
    padtap("shiftweapon 1",   eRt, rt);
    // Start (if the pad has one) also opens the in-game menu; the OUYA centre button is handled above.
    padtap("ingamemenu", eStart, start);

    // right stick -> mouse look (quadratic response for fine aim), applied every frame
    if(rx || ry)
    {
        float maxv = 32767.0f - gamepaddeadzone;
        float nx = rx / maxv, ny = ry / maxv;
        int dx = (int)(nx * (nx < 0 ? -nx : nx) * gamepadlooksens);
        int dy = (int)(ny * (ny < 0 ? -ny : ny) * gamepadlooksens);
...
void checkinput()
{
    ...
    while(events.length() || SDL_PollEvent(&event))
    {
        ...
        switch(event.type)
        {
            ...
            case SDL_KEYDOWN:
                if(!keyrepeatEnabled && event.key.repeat != 0) continue;
...
int main(int argc, char **argv)
{
    ...
    #endif
    #elif __ANDROID__
    // resolve the absolute base data dir (app external files dir) before any file access
    extern void setupandroidbasedir();
    setupandroidbasedir();
    // we only support landscape device orientation
    SDL_SetHint(SDL_HINT_ORIENTATIONS, "LandscapeLeft LandscapeRight");
    // OUYA/Android: make the soft-keyboard ENTER/Done key hide the IME. Without this hint SDL's
    // onNativeSoftReturnKey() is a no-op and the keyboard's Enter never reaches us at all (letters
    // arrive via commitText, but Enter is swallowed). With it set, Enter calls SDL_StopTextInput();
    // the touchui textview then commits the field on the SDL_IsTextInputActive() falling edge - which
    // is also how the hardware Back key (onNativeKeyboardFocusLost -> SDL_StopTextInput) gets committed.
    SDL_SetHint(SDL_HINT_RETURN_KEY_HIDES_IME, "1");
    // this setting doubles or triples the frame rate on android, tested with Samsung S7 on ac_arctic
    glHint(GL_BATCH_HINT_GL4ES, 1000);
    // this might improve the frame rate
...
#define STRINGIFY_(x) #x
    ...
    if(SDL_COMPILEDVERSION != SDL_VERSIONNUM(linked.major, linked.minor, linked.patch))
        clientlogf("SDL: compiled version " SDLVERSIONSTRING ", linked version %u.%u.%u", linked.major, linked.minor, linked.patch);
    ...
    initlog("video: misc");
    SDL_SetWindowTitle(screen, "AssaultCube");
    ...
    #ifdef __ANDROID__
    // OUYA/Tegra3 performance overrides. Exec'd last so it wins over saved/default/autoexec config, and
    // while still INIT_LOAD so the texture-quality cvars (texreduce/maxtexsize/trilinear) apply cleanly.
    execfile("config/mobile_gfx.cfg");
    #endif
    initing = NOT_INITING;
    uniformtexres = !hirestextures;
    ...
    showmenu("main");   // OUYA: open the classic PC-style main menu at boot (replaces the touch UI mainscene)
    ...
    for(;;)
    {
        ...
        if(touchenabled()) checktouchinput();
        else checkinput();
        processgamepad();   // OUYA/Android gamepad works regardless of the touch/desktop input path
        ...
        if(frames>3 && !minimized)
        {
            int scrw, scrh;
            SDL_GetWindowSize(screen, &scrw, &scrh);
            gl_drawframe(scrw, scrh, fps<lowfps ? fps/lowfps : (fps>highfps ? fps/highfps : 1.0f), fps, elapsed);
