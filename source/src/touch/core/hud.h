...
struct hud
{
    ...
    void draw(playerent *p)
    {
        ...
        extern int touchcontrols; // OUYA: hide on-screen gameplay controls (gamepad is used)
        extern gmenu *curmenu;    // scoremenu is already declared (void*) via protos.h
        turn_on_transparency(255); // includes GL_ENABLE(GL_BLEND)
        // OUYA: the touch menu button is redundant now that the classic PC menu opens via the Start
        // button; only draw it (and the help icon) when on-screen touch controls are explicitly enabled.
        if(touchcontrols) drawtouchicon(edgeair, edgeair, 3, 1, 255); // TOUCH_GAME_CORNER_TOP_LEFT_1 - open settings scene
        if(touchcontrols) drawtouchicon(movementcontrolcenter.x - iconsize/2, edgeair, 3, 2, 255/4); // TOUCH_GAME_CORNER_TOP_LEFT_2- open help scene

        if(menuvisible() && curmenu == scoremenu) // OUYA: only the scoreboard gets the touch equipment/voicecom overlay
        {
        ...
        else if(touchcontrols) // OUYA: gameplay touch controls hidden by default (gamepad replaces them)
        {
