...
struct input
{
    ...
    void checkinput()
    {
        ...
        extern int touchcontrols; // OUYA: when off, the touch UI is fully disabled (gamepad-only)
        SDL_Event event;
        int tdx = 0, tdy = 0;
        while(events.length() || SDL_PollEvent(&event)) {
            if(events.length()) event = events.remove(0);

            // OUYA: drain touch (finger) events without acting on them so the touch UI / touch menu can
            // never be triggered (e.g. by the controller touchpad). We still pump SDL_PollEvent so the
            // gamepad and window/quit events keep working. The classic PC menu is gamepad-driven instead.
            if(!touchcontrols && (event.type == SDL_FINGERMOTION || event.type == SDL_FINGERDOWN || event.type == SDL_FINGERUP))
                continue;
            ...
            switch(event.type) {
                ...
                case SDL_KEYUP:
                    ...
                    // OUYA: while the soft keyboard is up for a classic-menu text field, route its
                    // backspace / delete / enter / arrows into the focused field.
                    if(event.type == SDL_KEYDOWN && SDL_IsTextInputActive()) {
                        extern void menutextinputkey(int sym);
                        menutextinputkey((int) event.key.keysym.sym);
                    }
                    break;
                case SDL_TEXTINPUT:
                    // OUYA: feed soft-keyboard characters into the focused classic-menu text field.
                    if(SDL_IsTextInputActive()) {
                        extern void menutextinputchar(const char *s);
                        menutextinputchar(event.text.text);
                    }
                    // fall through so the touch UI still gets it when visible
