...
// true while an input-capturing menu is up (used by the OUYA gamepad mapping in main.cpp)
bool gamepadmenuopen() { return curmenu != NULL && curmenu->allowinput; }

// OUYA: text-entry helpers so the gamepad code can pop the Android soft keyboard for the currently
// selected classic-menu text field (nickname, custom IP, ...) and feed it the typed characters.
bool menutextinputfocused()
{
    if(!curmenu || !curmenu->allowinput) return false;
    if(!curmenu->items.inrange(curmenu->menusel)) return false;
    return curmenu->items[curmenu->menusel]->mitemtype == mitem::TYPE_TEXTINPUT;
}
void menutextinputchar(const char *s)   // route soft-keyboard text into the focused field
{
    if(!menutextinputfocused()) return;
    mitem *m = curmenu->items[curmenu->menusel];
    for(const char *c = s; *c; c++) m->key(0, true, (unsigned char)*c);
}
void menutextinputkey(int sym)          // route soft-keyboard backspace/enter/etc. into the focused field
{
    if(!menutextinputfocused()) return;
    curmenu->items[curmenu->menusel]->key(sym, true, 0);
}
void menutextinputcommit()              // apply the focused field's value (called when the soft keyboard closes)
{
    if(!menutextinputfocused()) return;
    curmenu->items[curmenu->menusel]->key(SDLK_RETURN, true, 0);   // mitemtextinput::key runs its action on RETURN
}
...
void showmenu(const char *name, bool top)
{
    ...
    // OUYA: classic PC menus are now allowed in touch mode too (gamepad-navigated). The acmobile
    // branch restricted this to the scoreboard only; we re-enable the full menu system for the port.
