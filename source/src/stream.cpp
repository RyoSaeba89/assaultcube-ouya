...
// On Android all engine paths must be absolute and rooted at the app's external files dir
// (where the Java side exported the game assets). The exact location is device-dependent
// (/storage/emulated/0/... on API17+, /mnt/sdcard/... on the API16 OUYA), so we resolve it at
// runtime via SDL_AndroidGetExternalStoragePath() instead of hardcoding it. See setupandroidbasedir().
const char *absbasedir = NULL;

#if defined(__ANDROID__) && !defined(STANDALONE)
void setupandroidbasedir()
{
    static string androidbasedir;
    const char *ext = SDL_AndroidGetExternalStoragePath();
    if(ext && *ext)
    {
        formatstring(androidbasedir)("%s/", ext);
        absbasedir = androidbasedir;
    }
    else absbasedir = "/storage/emulated/0/Android/data/net.cubers.assaultcube/files/";
}
#else
void setupandroidbasedir() {}
#endif
